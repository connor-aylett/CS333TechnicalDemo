...
def lloyds_kmeans(data, k, iterations):
    ...
    for i in range(iterations):
        ...
        clusterDistance(centroids, clusters)
...
def centroidDifference(centroids1, centroids2):
    sum = 0
    for cent in range(len(centroids1)):
        sum += distance(centroids1[cent], centroids2[cent])
...
def clusterDistance(centroids, clusters):
    for centerindex in range(len(centroids)):
        tempsum = 0
        for pointindex in range(len(clusters[centerindex])):
            tempsum += distance(centroids[centerindex],
...
def meandata(data):
    ...
    for col in range(cols):
        ...
        # The case for when the data is categorical (11 of the 150 columns)
            ...
            #categoricalAvg = round(categoricalAvg)

            temporary = 10
            smallestId = ""
            for id in m:
                number = m[id]
                diff = abs(number - categoricalAvg)
                if diff < temporary:
                    temporary = number
                    smallestId = id
                #if m[id] == categoricalAvg:
            #print(smallestId)
            means_and_categorical_modes.append(smallestId)
            
    #print("How many times else is entered per cluster: ", count)
...
totalresults = lloyds_kmeans(dataset, 3, 100)
